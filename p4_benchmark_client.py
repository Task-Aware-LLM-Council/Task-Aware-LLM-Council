...
# DeepSeek-R1-AWQ-2 outperformed Gemma-9B-GPTQ as synthesizer in our
# validation runs (0.40 EM vs 0.18 EM on MuSiQue). The docs/p4-report.md
# claims Gemma-9B was the baseline synthesizer that produced 0.75 EM, but
# that result is not reproducible from the current adapter+prompt+
# scoring stack. DeepSeek is the best-performing option we can ship.
...
_MUSIQUE_CONSTRAINED_PROMPT = (
    ...
    "general knowledge.\n"
    "2. The answer is ALWAYS hidden in the text. The question is multi-hop: "
    "identify the entities in one paragraph, then chain to their properties "
    "in another paragraph to derive the final answer.\n"
    "3. If you find a candidate answer (a name, number, date, or place) "
    "anywhere in the paragraphs that plausibly fits the question, COMMIT "
    "to it. Do not second-guess yourself because the evidence is not "
    "perfectly direct or because not every reasoning step has explicit "
    "textual support — multi-hop questions require some inference.\n"
    "4. Try multiple interpretations of the question if your first reading "
    "leads to a dead end. The mapping from question to relevant entities "
    "may not be obvious.\n"
    "5. Reserve 'NOT PRESENT IN CONTEXT' for the rare case where no "
    "candidate of the right type appears anywhere in the paragraphs after "
    "exhaustive search.\n\n"
    "Context:\n{context}\n\n"
    "Question: {question}\n\n"
    ...
    "Use 'NOT PRESENT IN CONTEXT' only as a last resort when no candidate "
    "exists in any paragraph."
)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Comma-separated SOURCE:ROLE overrides that bypass the router "
             ...
             "the reasoning specialist, not QA. FEVER intentionally omitted "
             "— learned router picks qa_reasoning (gemma-9B), which with "
             "the full-response label-scan scorer reaches the same 0.72 "
             "as Qwen-14B fact_general. Worth A/B'ing. "
