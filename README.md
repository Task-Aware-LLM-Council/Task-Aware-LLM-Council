...
P4's run/resume/score recipe is documented end-to-end in
[`packages/council_policies/src/council_policies/p4/P4_RUN.md`](packages/council_policies/src/council_policies/p4/P4_RUN.md).
That guide covers:

- mandatory environment setup (apptainer, PATH, HF cache pre-warm)
- `p4_benchmark_client.py` flags (`--limit`, `--chunk-size`, `--peft-adapter`, `--musique-oracle`)
- smoke testing on the `debug` partition before submitting a full run
- the resume loop (`p4_benchmark_client.py` is idempotent against its
  output JSONL — rerun the same `sbatch` to pick up where it stopped)
- scoring with `scratch/score_p4_results.py` (non-code datasets) and
  `scratch/score_humaneval_pass1.py` (HumanEval+ pass@1)
- diagnostic scripts in `scratch/` for routing analysis and
  per-dataset failure-mode triage

### Quick start

```bash
sbatch p4_rd2_val.sbatch
# … wait for completion or 2h timeout, then resume:
sbatch p4_rd2_val.sbatch     # picks up where the previous job stopped
# … repeat until p4_gemma_lora_v2_rd2_val.jsonl is full, then score:
