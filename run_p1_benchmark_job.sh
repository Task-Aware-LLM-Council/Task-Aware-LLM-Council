...
usage() {
  cat <<'EOF'
Usage:
  ./run_p1_benchmark_job.sh [options]
...
JOB_NAME="p1-bench-$(date +%Y%m%dT%H%M%S)"
...
sbatch \
  ...
  --export=ALL,QA_MODEL="$QA_MODEL",REASONING_MODEL="$REASONING_MODEL",GENERAL_MODEL="$GENERAL_MODEL",BASE_PORT="$BASE_PORT",ROLES="$ROLES",N_PER_DATASET="$N_PER_DATASET",DATASETS="$DATASETS",OUTPUT_ROOT="$OUTPUT_ROOT" \
  <<'EOF'
#!/bin/bash
set -euo pipefail
...
echo "=== P1 Individual Model Benchmark ==="
